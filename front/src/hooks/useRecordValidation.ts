...
function validateNumericField(value: string): string | undefined {
...
function validatePositiveNumericField(value: string): string | undefined {
  if (value === '') return '値を入力してください';
  const num = Number(value);
  if (Number.isNaN(num) || num <= 0) return '正しい数値を入力してください';
  return undefined;
...
function computeErrors(
  ...
  for (const row of cardios) {
    // Skip completely empty rows (they will be filtered out before submission)
