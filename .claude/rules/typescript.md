...
**検証ライブラリは未導入。** 入力検証は `lib/validation.ts` の**純粋関数**（`validateNumericField()` 等）と、Route Handler 内の型・必須チェックで行う。状態管理は `hooks/useRecordValidation.ts` が担う（`frontend.md`「ロジック分離」）。

- **新規の検証も、既存の `lib/validation.ts` に純粋関数として追加する。** 検証ライブラリを 1 箇所だけ持ち込んで**混在させない**。
