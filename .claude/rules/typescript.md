...
**検証ライブラリは未導入。** 入力検証は**純粋関数**（`validateNumericField()` 等）と、Route Handler 内の型・必須チェックで行う。状態管理は `hooks/useRecordValidation.ts` が担う（`frontend.md`「ロジック分離」）。

- 検証ライブラリを 1 箇所だけ持ち込んで**混在させない**。
- 純粋関数のままでも、**同じ入力ルールを 2 箇所に書かない**という原則は変わらない（`duplication.md`）。制約値は定数として共有する。

### 置き場所（ディレクトリを切る）

型・定数と同じく、**ソースルート直下にディレクトリを切る**。

| 構成 | ディレクトリ |
|---|---|
| Zod を使う | `src/schemas/` |
| **Zod を使わない（本プロジェクト）** | **`src/validation/`** |

- **`lib/validation.ts` のような単一ファイルにまとめない。** ディレクトリを切り、ドメイン単位でファイルを分ける（`validation/record.ts` 等）。
- **`lib/` `utils/` の下に置かない**（「関数の置き場」と分離する）。
- **検証を伴わない純粋な型はここに置かない**（`types/` へ）。置くのは「実行時に検証するもの」だけ。
- barrel（`index.ts`）は作らない（理由は型・定数と同じ）。
- **現状は `lib/validation.ts`（単一ファイル）** で、この規定を満たしていない。移行は issue #111 で対応する。

### 将来方針（検証ライブラリを導入する場合）

**導入するなら Zod に統一する。** フロントエンド・Route Handler で**同じ 1 つのライブラリに揃える**（`class-validator` / `yup` / `joi` と混在させない）。導入は `lib/validation.ts` の置き換えとして**一括で行い**、純粋関数と Zod スキーマが併存する期間を残さない。
...
- 集約先は**ソースルート直下の `types/` ディレクトリ**。本プロジェクトは `src/` 構成のため **`front/src/types/`**。
- **単一ファイルにまとめない。** `lib/type.ts` / `src/types.ts` のように 1 ファイルへ全型を詰め込む形は禁止。必ず**ディレクトリを切り、ドメイン単位でファイルを分ける**。
- **`lib/` `utils/` の下に型ファイルを置かない。** `lib/` は「関数の置き場」、`types/` は「型の置き場」で分離する（定数・スキーマも同様）。
- 命名は**ディレクトリが複数形の `types`、ファイルはドメイン名の単数形**（`types/master.ts`）。`type.ts` / `Types.ts` のような単数形・PascalCase のディレクトリ名は使わない。
...
- 集約先は**ソースルート直下の `constants/` ディレクトリ**（本プロジェクトは `front/src/constants/`）。**単一ファイルにまとめず**、ドメイン単位でファイルを分ける。
