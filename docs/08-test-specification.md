...
- ユニットテスト: バリデーション/カロリー/フック/API を High 優先度で網羅（目安件数は test-design 参照）。
<!-- 定量カバレッジ目標（行・分岐）は未確定 -->

## 使用ツール

- ユニット: Vitest 4（jsdom, `@testing-library/react`）。設定: `front/vitest.config.ts`, `front/src/test/setup.ts`。
- E2E: Playwright（`front/tests/e2e/`、`smoke.spec.ts` / `record-crud.spec.ts`）。
- 静的検査: ESLint（`eslint-plugin-jsdoc` 含む）+ `tsc --noEmit` + `next build`。CI の `static-check` ジョブで実行。
- CI: GitHub Actions（`.github/workflows/test.yml`、`static-check` / `unit-test` / `e2e-test` ジョブを並列実行）。
